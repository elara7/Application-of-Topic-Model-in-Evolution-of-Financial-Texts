...
corpus_path = main_path + 'Elara/Documents/paper/5_0_filter/corpus/'
split_path = main_path + 'Elara/Documents/paper/6_cluster/cluster_res/result_raw_split/'
...
q2 = 'select \
    ...
    case when is_split_2016 in (1,2,3) or is_split_all in (1,2,3) then 1 else 0 end as is_split \
    from \
    (\
    select \
            name,\
...
def fill_tag(df):
    ...
    a1,a2,a3,n1,n2,n3,f = 1,1,1,1,1,1,0
    #11 12 13  14 15 16
    l = max(df['date'])
    for i in range(len(df)):
        if i == 0:
            da = df.iloc[i,1]
            start = datetime.strptime(str(df.iloc[i,1]),'%Y%m%d')
            start_index = 0
            end = datetime.strptime(str(df.iloc[i,1]),'%Y%m%d')
            end_index = 0
        ...
        if ((not math.isnan(df.iloc[i,17])) and df.iloc[i,17] != 0) or df.iloc[i,1]==l:
            if (datetime.strptime(str(df.iloc[i,1]),'%Y%m%d')-end).days>=30 :
                ...
                end_index = i
                if (end - start).days >= 30 or df.iloc[i,1]==l:
                    f+=1
                    for x in range(start_index,end_index+1):
                        df.iloc[x,17] = f
                    end = datetime.strptime(str(df.iloc[i,1]),'%Y%m%d')
...
if __name__ == '__main__':
    ...
    print('tail 10:')
    print(dfk.tail(10))
    dfk.to_csv(main_path+'Elara/Documents/paper/7_split_corpus/'+'split_result.csv',encoding='utf-8',header=False,index=False)
    print('save ok')
