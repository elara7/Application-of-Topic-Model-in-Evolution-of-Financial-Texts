...
lda_best_model_path = main_path2 + 'lda_run/bestmodel/'
dict_path = main_path2 + 'lda_run/dic/'
...
def get_topic_score(model_info,text,dictionary,stage=0,stock_name='上汽集团',word_top_n=None,tfidf_top_n=10):
    ...
    grab = [[i,grab[i]/max(grab)] for i in range(len(grab))]
    grab_score = grab
    ...
    texts = [text.iloc[i,5].split() for i in range(len(text))]
...
#        dist_score.append([i,high_f_word_time[i]/max(high_f_word_time)])
    ...
    for ind in range(topic_num):
        ...
        for j in i:
            if tfidf_top_n!=None:
                if t>=tfidf_top_n:
                    break
            if re.match(r'(快速发展)|(.*上汽.*)|(.*集团.*)|(汽车)|(公司)|(亿.)|(万.)|(股市场)|(的竞争)|(.*个股)|(领域)|(的.*)',j[0]) == None:
    ...
    topic_score = [[i,grab_score[i][1],active_score[i][1],word_tfidf[i],word[i]] for i in range(topic_num) if grab_score[i][1]>0 and active_score[i][1]>0  and len(word_tfidf[i].split())>5]
...
def get_result(stock_name):
    ...
    text_info = pd.read_csv(tfidf_filter_corpus_path + stock_name+'.csv',encoding='utf-8',engine='python',names = ['stage','name','date','title','url','content','ch','source','url_ch'])
    
    last_stage_info = []
    for stage in stage_list:
        try:
            temp = text_info.loc[text_info['stage']==stage]
            text = temp.sort_values(by=['date','url'])
            ...
            topic_info.to_csv(main_path2 + 'lda_run/topic_info/'+stock_name+'_'+str(stage)+'.csv',encoding='UTF-8',header=None,index=None)
            if len(topic_info)==0:
            ...
            if len(last_stage_info)==0:
                ...
                for i in range(len(last_stage_info[0])):
                    for j in range(i+1,len(topic_info)):
                        ...
                        if spearman_res[1]==-1 or spearman_res[0]<0.5 :
                            continue
                        else:
                            spearman_res_table.append(spearman_res+[topic1_id,last_stage_info[0]['word_tfidf'].iloc[i],topic1_str,topic2_id,topic_info['word_tfidf'].iloc[i],topic2_str])
                if len(spearman_res_table)==0:
                    last_stage_info = [topic_info,stage]
                    continue
                spearman_res_table=pd.DataFrame(spearman_res_table)
                spearman_res_table.to_csv(main_path2+'lda_run/topic_track/'+stock_name+'_'+str(last_stage_info[1])+'_'+str(stage)+'.csv',encoding='UTF-8',header=None,index=None)
                last_stage_info = [topic_info,stage]
        except:
            traceback.print_exc()
...
if __name__ == '__main__':
    ...
    res = pool.map(get_result,stock_names)       