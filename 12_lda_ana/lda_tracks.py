...
def get_result(stock_name):
    ...
    for stage in stage_list:
        try:
            ...
            topic_info.to_csv(main_path2 + 'lda_run/topic_info_new/'+stock_name+'_'+str(stage)+'.csv',encoding='UTF-8',header=None,index=None)
            if len(topic_info)==0:
                ...
                for i in range(len(last_stage_info[0])):
                    for j in range(len(topic_info)):
                        ...
                        if spearman_res[1]==-1 or spearman_res[0]<=0 :
                            ...
                            spearman_res_table.append(spearman_res+[topic1_id,last_stage_info[0]['active_score'].iloc[i],last_stage_info[0]['word_tfidf'].iloc[i],topic1_str,topic2_id,topic_info['active_score'].iloc[j],topic_info['word_tfidf'].iloc[j],topic2_str])
                if len(spearman_res_table)==0:
                    last_stage_info = [topic_info,stage]
                    print(stage)
                    continue
                spearman_res_table=pd.DataFrame(spearman_res_table)
                spearman_res_table.to_csv(main_path2+'lda_run/topic_track_new/'+stock_name+'_'+str(last_stage_info[1])+'_'+str(stage)+'.csv',encoding='UTF-8',header=None,index=None)
                last_stage_info = [topic_info,stage]
        except:
            traceback.print_exc()
