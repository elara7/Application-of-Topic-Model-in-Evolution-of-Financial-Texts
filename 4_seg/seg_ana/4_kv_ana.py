...
merged_kv_corpus_path = main_path+ 'Elara/Documents/paper/4_seg/kv/kv.csv'


print('load merge kv corpus')

    
merge_kv_corpus = pd.read_csv(merged_kv_corpus_path, names=['name','content'])
...
all_frequency.to_csv(main_path+'Elara/Documents/paper/4_seg/kv/res/merged_frequency_all.csv',header=None,index=None)
...
for n in range(len(names)):
    ...
    all_frequency.columns=['word','cnt']
    all_frequency = all_frequency.sort_values(by='cnt',ascending=False)
    all_frequency.to_csv(main_path+'Elara/Documents/paper/4_seg/kv/res/stocks/merged_frequency_'+names[n]+'.csv',header=None,index=None)
