...
def train_lda(name,stage,corpus,dictionary,numoftopic):
    try:
        ...
        lda_fit.save(main_path2 + 'lda_run/resultmodel/'+name+'_'+str(stage)+'_'+str(doc_len)+'_'+str(numoftopic) ,ignore=('state', 'dispatcher','id2word'))
        return lda_fit
    except:
...
def find_best_k(name,stage,corpus,dictionary):
    ...
    for i in range(6):
        ...
        if np.abs(small-large)<=1:
            break
    last_rep_model.save(main_path2 + 'lda_run/bestmodel/'+name+'_'+str(stage)+'_'+str(doc_len)+'_'+str(numoftopic) ,ignore=('state', 'dispatcher','id2word'))
    return None
...
def lda_run(stockname):
    ...
    for i in stage_list:
        ...
        try:
            ...
            dictionary.save(main_path2 + 'lda_run/dic/'+name+str(stage))
        except:
