...
class test_f(object):
    ...
    def __init__(self,text_train,text_test,dictionary):
    ...
    def test_f(self,line):
        ...
        nt = line[0]
        it = line[1]
        Perplexity = gensim.models.callbacks.PerplexityMetric(self.text_test,logger='shell')
        ...
        for x in range(len(d)):
            for y in range(x+1,len(d)):
                if d[y,x]==0:
                    is_rep=1
                    break
        print('训练文本数',self.cl,'测试文本数',self.tl,'字典长度',self.dl,'主题数',nt,'迭代次数',it,'有重复',is_rep)
        return [self.cl]+[self.tl]+[self.dl]+[nt]+[it]+[is_rep]+lda.metrics['Perplexity']       
...
for i in list(set(test_text['i'])):
    ...
    print(len(text_test))
    ttt = test_f(text_train,text_test,dictionary)
    
        
    temp = pool.map(ttt.test_f,para)
    
    pd.DataFrame(temp).to_csv(main_path+'Elara/Documents/paper/LDA/'+str(i)+'lda_test_res_test_set.csv',header=None,index=None)